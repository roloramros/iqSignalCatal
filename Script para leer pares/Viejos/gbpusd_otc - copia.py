...
def verificar_integridad_velas():
    ...
    response = supabase.table("gbpusd_otc") \
        .select("*") \
...
supabase.table("gbpusd_otc").delete().neq("id", 0).execute()
...
for c in candles:
    ...
    supabase.table("gbpusd_otc").insert(data).execute()
if candles:
