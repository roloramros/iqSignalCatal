...
def verificar_integridad_velas():
    ...
    response = supabase.table("eurgbp_otc") \
        .select("*") \
...
supabase.table("eurgbp_otc").delete().neq("id", 0).execute()
...
for c in candles:
    ...
    supabase.table("eurgbp_otc").insert(data).execute()
if candles:
