...
def verificar_integridad_velas():
    ...
    response = supabase.table("audcad_otc") \
        .select("*") \
...
supabase.table("audcad_otc").delete().neq("id", 0).execute()
...
for c in candles:
    ...
    supabase.table("audcad_otc").insert(data).execute()
if candles:
