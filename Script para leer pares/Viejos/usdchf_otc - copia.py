...
def verificar_integridad_velas():
    ...
    response = supabase.table("usdchf_otc") \
        .select("*") \
...
supabase.table("usdchf_otc").delete().neq("id", 0).execute()
...
for c in candles:
    ...
    supabase.table("usdchf_otc").insert(data).execute()
if candles:
